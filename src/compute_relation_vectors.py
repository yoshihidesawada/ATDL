...
def modify_relation_vectors(init_vectors, predict_vectors, labels):
	...
	covs = np.zeros((mc._TARGET_DIM_NUM, mc._SOURCE_DIM_NUM,mc._SOURCE_DIM_NUM), dtype=np.float32)
	invs = np.zeros((mc._TARGET_DIM_NUM, mc._SOURCE_DIM_NUM,mc._SOURCE_DIM_NUM), dtype=np.float32)
	for j in range(mc._TARGET_DIM_NUM):
	...
	for j in range(mc._TARGET_DIM_NUM):
		best_y = 0.0
		for k in range(mc._TARGET_DIM_NUM):
			if j != k:
...
def compute_relation_vectors(nn, data, labels, fold_num, method_flag):
	...
	relation_vectors = np.zeros((mc._TARGET_DIM_NUM,mc._SOURCE_DIM_NUM), dtype=np.float32)
	lab_num = np.zeros(mc._TARGET_DIM_NUM, dtype=np.int32)
	...
	if fold_num == 1 and method_flag == mc._MODIFIED_MEAN_ATDL:
		...
		save_data.to_csv('%s'%(mc._MEAN_RELATION),index=False,header=False)
	...
	if fold_num == 1 and method_flag == mc._MODIFIED_MEAN_ATDL:
		save_data = pd.DataFrame(relation_vectors)
		save_data.to_csv('%s'%(mc._MODIFIED_MEAN_RELATINON),index=False,header=False)
...
def compute_relation_labels(nn, data, labels, fold_num):
	...
	if fold_num == 1:
		cidxs = np.zeros((mc._TARGET_DIM_NUM,mc._SOURCE_DIM_NUM), dtype=np.int32)
		for j in range(mc._TARGET_DIM_NUM):
			bak = df_predict_vectors[df_predict_vectors['lab']==j]
			bak = bak.drop('lab', axis=1)
			idx = np.argmax(bak.values, axis=1)
			cidx = collections.Counter(idx)
			for i in range(mc._SOURCE_DIM_NUM):
				cidxs[j][i] = cidx[i]

		save_data = pd.DataFrame(cidxs)
		save_data.to_csv('%s'%(mc._COUNT_RELATINON),index=False,header=False)
